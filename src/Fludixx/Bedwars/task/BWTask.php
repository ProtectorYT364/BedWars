        ...
        foreach (Bedwars::$arenas as $name => $arena) {
            if((count($arena->getPlayers()) >= (int)$arena->getPlayersProTeam()+1) and $arena->getCountdown() !== 0) {
                ...
                foreach ($arena->getPlayers() as $player) {
                    ...
                    $sb->setLine(1, "§lTeam: ".Utils::ColorInt2Color(Utils::teamIntToColorInt($mplayer->getPos())));
                    $sb->setLine(2, "\0");
                    $i = 3;
                ...
                $sb = new Scoreboard($name);
                $sb->setTitle("§l§e$name");
                $sb->setLine(1, "§lTimer: §b".$arena->getCountdown());
                $sb->setLine(2, "§lPlayers: §a".(count($arena->getPlayers()))."§f / §c".($arena->getPlayersProTeam()+1));
                foreach ($arena->getPlayers() as $player) {
                    $mplayer = Bedwars::$players[$player->getName()];
                    $mplayer->sendScoreboard($sb);
                }
